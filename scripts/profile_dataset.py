...
def enrichment_figures() -> None:
    ...
    # The dbt docs site, published by the `pages` job from the same build whose
    # tests passed. Same rule as the demo URL and for the same reason: a link
    # that 404s is worse than a sentence saying it is not published yet.
    docs_url = deployment["dbt_docs_url"]
    fig(
        "dbt_docs_link",
        f"**[Browse the dbt docs]({docs_url})** — every model, column, test and "
        "the full lineage graph, generated from the build."
        if docs_url
        else "The dbt docs site is generated by CI on every push to `main` "
        "(`dbt docs generate`, uploaded as a Pages artifact) and **is published and "
        "serving**. The link is held back until GitHub finishes provisioning the "
        "HTTPS certificate for the domain — publishing an `http://` link in a "
        "README is the kind of detail that is true today and embarrassing next "
        "week. `make docs` builds and serves it locally in the meantime.",
    )
