...
The dbt docs site is generated by CI on every push to `main` (`dbt docs generate`, uploaded as a Pages artifact) and **is published and serving**. The link is held back until GitHub finishes provisioning the HTTPS certificate for the domain — publishing an `http://` link in a README is the kind of detail that is true today and embarrassing next week. `make docs` builds and serves it locally in the meantime.
